...
class ExperienceBuffer:
    ...
    def _create_tensor_from_space(self, space, base_shape):
        ...
        print("EXPERIENCE_INIT")

        # If the tensor is simply a tuple...

        # [Horizon, Env_i, H, W, C]
        return torch.zeros(base_shape + space, dtype=torch.float32, device = self.device)
...
class MultiModalExperienceBuffer:
    '''
    The experience buffer can hold data of different modalities.
    Implemented for on-policy algos
    '''
    ...
    def _init_from_env_info(self, env_info):
        obs_base_shape = self.obs_base_shape
        state_base_shape = self.state_base_shape
        assert 'modality' in env_info

        modality_dict = env_info['modality'] # {modality_name [str]: modality_shape [tuple]}

        for modality_name, modality_shape in modality_dict.items():
            self.tensor_dict[modality_name] = self._create_tensor_from_space(modality_shape, obs_base_shape)
            print("Experience Buffer Modality: {}".format(modality_name),
                  'shape:', self.tensor_dict[modality_name].shape)

        # if 'modality_shape' not in env_info:
        #     self.tensor_dict['obses'] = self._create_tensor_from_space(env_info['observation_space'], obs_base_shape)
        # else:
        #     print("Initialization using madality shape")
        #     self.tensor_dict['obses'] = self._create_tensor_from_space(env_info['modality_shape'], obs_base_shape)
        ...
        if self.is_discrete or self.is_multi_discrete:
            self.tensor_dict['actions'] = self._create_tensor_from_space(
                gym.spaces.Box(low=0, high=1, shape=self.actions_shape, dtype=np.long), obs_base_shape)
        if self.use_action_masks:
            self.tensor_dict['action_masks'] = self._create_tensor_from_space(
                gym.spaces.Box(low=0, high=1, shape=self.actions_shape + (np.sum(self.actions_num),), dtype=np.bool),
                obs_base_shape)
        if self.is_continuous:
            self.tensor_dict['actions'] = self._create_tensor_from_space(
