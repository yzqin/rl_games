...
def behavioral_cloning_actor_loss(action_neglog_probs):
    a_loss = action_neglog_probs
    return a_loss


def ppo_behavioral_cloning_actor_loss(action_neglog_probs, old_action_neglog_probs, curr_e_clip=0.2):
    ratio = torch.exp(old_action_neglog_probs - action_neglog_probs)
    surr1 = ratio
    surr2 = torch.clamp(ratio, 1.0 - curr_e_clip, 1.0 + curr_e_clip)
    a_loss = torch.max(-surr1, -surr2)
